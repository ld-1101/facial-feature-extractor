...
| 工具 | 建议用法 |
|------|----------|
| **Seedream** | `generate_prompt.py` 一键生成精准提示词 |
| **Stable Diffusion** | 把 DNA JSON 里的 `description` 字段直接当 prompt 用 |
| **Midjourney** | 复制 `description` 或用 `generate_prompt.py` 输出 |
| **ComfyUI** | 把 JSON 节点接入 Workflow，面部参数精确控制 |
| **ChatGPT / Claude** | 贴入 DNA JSON，让它帮你改写为任意风格描述 |
