...
基于 **MIMO v2.5 视觉模型** + **人脸关键点几何测量体系**（参照 dlib 68点 / MediaPipe 468点）的人脸特征提取器。

从照片中自动提取 **80+ 个可量化的人脸几何维度**，生成可供 AI 生图模型（Seedream / Stable Diffusion / Midjourney）直接使用的精确面部描述。

## 核心能力

- 📐 **几何测量** — 眼裂长度(mm)、眦角倾斜度(°)、鼻唇角(°)、面长面宽比等精确数值
- 🔍 **双模式分析** — 清晰人脸完整提取 / 模糊人脸部分提取 + 默认模板补齐
- 📝 **提示词生成** — 自动将 DNA JSON 转为 Seedream/Stable Diffusion 可直接使用的提示词
...
# 或创建 .env 文件: MIMO_API_KEY=tp-xxxxxxxx
```
...
```json
{
  ...
  "upper_lip_thickness_mm": 7,
  "lower_lip_thickness_mm": 10,
  "oral_commissure_angle_degrees": 4,
  "skin_tone": "冷白皮(偏粉调)",
  "hair_color_hex": "#4A3629",
  "height_cm": 168,
  "waist_hip_ratio": 0.68,
  "temperament_primary": "东方初恋脸(温柔甜美精致灵气)",
...
参照人脸识别领域标准：
- **dlib 68-point** 面部关键点（iBUG 300-W 数据集）
